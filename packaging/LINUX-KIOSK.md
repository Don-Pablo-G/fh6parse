...
10. **WARNING:** lines: `H{n} does not match T{tool}` on G43, `D{n} does not match T{tool}` on any D, `G95 still active…` if feed-per-rev was not cancelled with G94 before the next tool (or M30), empty-pocket (`Txx M6` with no motion) except the **last** tool change (spindle prep), and `G55 after operation started (L…)` if G54–G59 appears after the first Txx M6 or M97/M98 (offset belongs in the file preamble only). Matching H/D stay quiet. Comments with `!` print as **Programmer notes**.
