...
6. **Red RUN** (right) — full slip (old FULL): STEP, offset block, ops, cycle + time split (G0 / cut / rot / canned / probe / ATC stacked per T), tool list, each Txx M6 / M00, all warnings. The three colour chips on the bottom of the screen sit in this same left→right order. After a ticket, LOAD / SET / RUN are ignored for **print wait** (default 2 s; **Czekaj przed następnym biletem** / **Wait before the next ticket**) so a second press does not cut two slips.
